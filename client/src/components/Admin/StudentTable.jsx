...
import { Mail, GraduationCap, Edit2, Trash2, ClipboardCheck, Lock, Eye, EyeOff } from 'lucide-react';

const StudentTable = ({ students, onEdit, onDelete, onManageMarks }) => {
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {students.length > 0 ? students.map((s) => (
              <tr key={s._id} className="hover:bg-gray-50/50 transition group">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-mono text-gray-600 bg-gray-50 px-2 py-1 rounded border border-gray-100 min-w-[100px] text-center">
                      {showPasswords[s._id] ? (s.password || 'password123') : '••••••••'}
                    </span>
                    <button 
                      onClick={() => togglePassword(s._id)}
                      className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition"
                      title={showPasswords[s._id] ? "Hide Password" : "Show Password"}
                    >
                      {showPasswords[s._id] ? <EyeOff size={16} /> : <Eye size={16} />}
                    </button>
                  </div>
                </td>
                <td className="px-6 py-4">
