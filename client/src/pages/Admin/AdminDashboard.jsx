...
const AdminDashboard = () => {
  ...
  // Modals State
  const [userModal, setUserModal] = useState({ open: false, mode: 'student', user: null });
  const [marksModal, setMarksModal] = useState({ open: false, student: null });
  const [subjectModal, setSubjectModal] = useState({ open: false, faculty: null });
  ...
  const handleUserSubmit = async (formData) => {
    try {
      ...
      alert(err.response?.data?.message || 'Failed to process user request. Note: The backend update route might be missing.');
    }
  };
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-gray-500 font-medium">View and manage student records and marks.</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                onClick={() => setUserModal({ open: true, mode: 'student', user: null })}
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-gray-500 font-medium">Manage faculty assignments and subjects.</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                onClick={() => setUserModal({ open: true, mode: 'faculty', user: null })}
          ...
          <FacultyTable 
            ...
            onEdit={(u) => setUserModal({ open: true, mode: 'edit', user: u })}
            onDelete={handleDeleteUser}
            onManageSubjects={(u) => setSubjectModal({ open: true, faculty: u })}
          />
        </div>
      );
    }
      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 font-medium">Manage your educational ecosystem efficiently.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className="text-3xl font-black text-indigo-600 mt-2">{users.filter(u => u.role === 'STUDENT').length}</p>
            <p className="text-sm text-gray-400 mt-1 font-medium">Manage records and marks</p>
          </div>
          ...
          >
            ...
            <p className="text-3xl font-black text-blue-600 mt-2">{users.filter(u => u.role === 'FACULTY').length}</p>
            <p className="text-sm text-gray-400 mt-1 font-medium">Assign subjects and roles</p>
          </div>
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl transition-all group">
            ...
            <p className="text-3xl font-black text-purple-600 mt-2">{users.filter(u => u.role === 'ADMIN').length}</p>
            <p className="text-sm text-gray-400 mt-1 font-medium">System administrators</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <div className="flex items-start gap-4 p-4 rounded-xl bg-gray-50 border border-gray-100 hover:bg-indigo-50 transition-colors cursor-pointer" onClick={() => navigate('/admin/students')}>
                <div className="p-2 bg-white rounded-lg shadow-sm text-indigo-600">
                  <UserPlus size={20} />
                </div>
                <div>
                  <h4 className="font-bold text-gray-800">Quick Add Student</h4>
                  <p className="text-sm text-gray-500">Register new students to the system.</p>
                </div>
              </div>
              <div className="flex items-start gap-4 p-4 rounded-xl bg-gray-50 border border-gray-100 hover:bg-blue-50 transition-colors cursor-pointer" onClick={() => navigate('/admin/faculty')}>
                <div className="p-2 bg-white rounded-lg shadow-sm text-blue-600">
                  <UserPlus size={20} />
                </div>
                <div>
                  <h4 className="font-bold text-gray-800">Manage Faculty</h4>
                  <p className="text-sm text-gray-500">View and update faculty subject assignments.</p>
                </div>
              </div>
            </div>
          </div>
    ...
    <div className="pb-12">
      {loading && users.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24">
          ...
          <p className="mt-6 text-gray-500 font-bold text-lg">Synchronizing data...</p>
        </div>
      ) : error ? (
        <div className="bg-red-50 border border-red-100 text-red-600 p-8 rounded-2xl text-center shadow-sm">
          <ShieldCheck size={48} className="mx-auto mb-4 opacity-50" />
          <p className="text-lg font-bold">{error}</p>
          <button onClick={fetchUsers} className="mt-4 px-6 py-2 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition">Try Again</button>
        </div>
      ) : (
        renderContent()
      )}
